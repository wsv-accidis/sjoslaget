...
class CruiseRepository {
  ...
  Future<List<CruiseCabin>> getActiveCruiseCabins(Client client) async {
    ...
    return jsonBody.map((dynamic value) => CruiseCabin.fromMap(value)).toList();
  }
  ...
  Future<List<CruiseProduct>> getActiveCruiseProducts(Client client) async {
    ...
    HttpStatus.throwIfNotSuccessful(response);
    final List jsonBody = json.decode(response.body);
    return jsonBody.map((dynamic value) => CruiseProduct.fromMap(value)).toList();
  }

  Future<List<SubCruise>> getActiveSubCruises(Client client) async {
    Response response;
    try {
      response = await client.get('$_apiRoot/cruise/activesubs');
    } on ExpirationException {
      rethrow; // special case for OAuth2 expiration
    } catch (e) {
      throw IOException.fromException(e);
    ...
    HttpStatus.throwIfNotSuccessful(response);
    final List jsonBody = json.decode(response.body);
    return jsonBody.map((dynamic value) => SubCruise.fromMap(value)).toList();
  }
