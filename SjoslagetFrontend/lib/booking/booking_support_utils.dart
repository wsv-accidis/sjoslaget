...
class BookingSupportUtils {
  static Future<Tuple2<BookingResult, String>> saveBooking(ClientFactory clientFactory, BookingRepository bookingRepository, BookingDetails bookingDetails,
    ...
    try {
      ...
      if (e is AvailabilityException) {
        ...
        bookingError =
            'Någonting gick fel när din bokning skulle sparas. Kontrollera att alla uppgifter är riktigt angivna och försök igen. Om problemet kvarstår, kontakta Sjöslaget.';
      } else {
        // Exception which is not coming from backend, potentially bad network
        bookingError =
            'Någonting gick fel när din bokning skulle sparas. Kontrollera att du är ansluten till internet och försök igen. Om problemet kvarstår, kontakta Sjöslaget.';
      }
  ...
  static int _getNumberOfCabinsOfType(List<BookingCabin> cabins, String id) {
    if (null == cabins) return 0;

    return cabins.where((b) => b.cabinTypeId == id).length;
  }

  static int _getProductQuantity(List<BookingProduct> products, String id) {
    if (null == products) return 0;
