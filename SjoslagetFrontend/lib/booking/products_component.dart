...
class ProductsComponent implements BookingAddonProvider, OnInit {
  ...
  int getAvailability(String id) {
    if (null == _availability || !_availability.containsKey(id)) return NOT_LIMITED;
  ...
  void validate() => bookingProducts.forEach(_bookingValidator.validateProduct);

  void _setQuantitiesFromBooking() {
    if (null == bookingProducts) return;
    ...
    if (null == _quantitiesFromBooking) {
      ...
      bookingProducts.forEach((bookingProductView) {
        final bookingProduct = _quantitiesFromBooking.firstWhere((bp) => bp.productTypeId == bookingProductView.id, orElse: () => null);
